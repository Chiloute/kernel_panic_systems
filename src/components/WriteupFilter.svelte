<script lang="ts">
  ...
  const allCats = $derived(
    [...new Set(writeups.map((w) => w.categorie))].sort(),
  );
...
<div class="flex flex-wrap gap-6 mb-8">
  <!-- Par CTF -->
  <div>
    <p class="font-mono text-xs text-zinc-600 uppercase tracking-widest mb-2">
      CTF
    </p>
    <div class="flex flex-wrap gap-1.5">
      {#each allCtfs as ctf}
        <button
          ...
          class="px-3 py-1 rounded font-mono text-xs border transition-all {activeCtf ===
          ctf
    ...
    <div class="flex flex-wrap gap-1.5">
      {#each allCats as cat}
        <button
          ...
          class="px-3 py-1 rounded font-mono text-xs border transition-all {activeCat ===
          cat
    ...
    <div class="flex flex-wrap gap-1.5">
      {#each ["easy", "medium", "hard", "insane"] as diff}
        <button
          ...
          class="px-3 py-1 rounded font-mono text-xs border transition-all {activeDiff ===
          diff
            ? diffColors[diff]
            : 'bg-transparent border-zinc-800 text-zinc-500 hover:border-zinc-600 hover:text-zinc-300'}"
        >
    ...
    {#each filtered as writeup (writeup.id)}
      ...
      >
        <div class="flex items-center gap-2 mb-3">
          <span
            class={`text-xs px-2 py-0.5 rounded font-mono border ${diffColors[writeup.difficulte]}`}
          >
            {writeup.difficulte}
          </span>
          <span class="text-xs text-zinc-500 font-mono"
            >{writeup.categorie}</span
          >
          {#if writeup.points}
            <span class="text-xs text-zinc-600 font-mono ml-auto"
              >{writeup.points} pts</span
            >
          {/if}
        </div>

        <h3
          class="font-bold text-zinc-100 group-hover:text-red-400 transition-colors mb-1 line-clamp-2"
        >
          {writeup.titre}
        </h3>
        ...
        <div class="flex flex-wrap gap-1">
          {#each writeup.tags.slice(0, 3) as tag}
            <span
              class="text-xs px-2 py-0.5 bg-zinc-800 text-zinc-400 rounded font-mono"
            >
              {tag}
            </span>
          {/each}
...
{:else}
  <p class="text-zinc-700 font-mono text-sm">
    // Aucun writeup pour ces critères
  </p>
{/if}
